    ...
    <div class="flex w-full items-center justify-between py-6 lg:border-none">
      ...
      <div class="flex items-center">
        <div class="ml-10 hidden space-x-8 lg:block">
          <button
            on:click="{() => handleHome()}"
            class="bg-transparent px-4 py-2 text-black hover:bg-lime-400 hover:font-black hover:uppercase hover:text-black hover:shadow-hard"
            >Home</button>
          <button
            on:click="{() => handleView()}"
            class="bg-transparent px-4 py-2 text-black hover:bg-lime-400 hover:font-black hover:uppercase hover:text-black hover:shadow-hard"
      ...
      <div>
        {#if $principal}
          <div
            class="ml-1 flex items-center gap-x-2 rounded-xl bg-gray-200 px-4 py-2 text-sm text-zinc-700">
            <svg
              ...
              ></path
              ></svg>
            {window.ic.plug.principalId.substring(
              window.ic.plug.principalId.length - 9,
            )}
          </div>
        {:else}
          <button
            class="mr-1 flex items-center px-4 py-2 font-semibold text-black hover:bg-lime-400 hover:font-black hover:uppercase hover:text-black hover:shadow-hard"
