...
shared ({ caller = initializer }) actor class Ledger() = this {
  ...
  let minting_account = {
    owner : Principal = initializer;
    subaccount : ?Blob = ?defaultSubaccount;
  };
  ...
  func makeGenesisChain() : TxLog {
    ...
    let myAccount : Account = {
      owner = Principal.fromText("7ohni-sbpse-y327l-syhzk-jn6n4-hw277-erei5-xhkjr-lbh6b-rjqei-sqe");
      subaccount = ?defaultSubaccount;
    };
    let myAccountB : Account = {
      owner = Principal.fromText("xi6k6-vxxki-pa4lb-hjsat-vcsff-jql77-ynywr-fw3er-nreex-hnt5a-hqe");
      subaccount = ?defaultSubaccount;
    };

    // validateSubaccount(account.subaccount);
    let tx : Transaction = {
      operation = #Mint({ spender = minting_account.owner; source = #Init; from = minting_account; to = minting_account; amount = 100000000; fee = null; memo = null; created_at_time = ?now });
      fee = 0;
      timestamp = now;
    };
    let tx1 : Transaction = {
      operation = #Mint({ spender = myAccount.owner; source = #Init; from = myAccount; to = myAccount; amount = 100000000; fee = null; memo = null; created_at_time = ?now });
      fee = 0;
      timestamp = now;
    };
    let tx2 : Transaction = {
      operation = #Mint({ spender = myAccountB.owner; source = #Init; from = myAccountB; to = myAccountB; amount = 10000000000; fee = null; memo = null; created_at_time = ?now });
      fee = 0;
      timestamp = now;
    };
    log.add(tx2);
  ...
  public shared ({ caller }) func icrc1_transfer({ from_subaccount : ?Subaccount; to : Account; amount : Tokens; fee : ?Tokens; memo : ?Memo; created_at_time : ?Timestamp }) : async Result<TxIndex, TransferError> {
    applyTransfer({ spender = caller; source = #Icrc1Transfer; from = { owner = caller; subaccount = from_subaccount }; to = to; amount = amount; fee = fee; memo = memo; created_at_time = created_at_time });
  };

  public shared ({ caller }) func icrc1_balance_of(account : Account) : async Tokens {
    balance(account, log);
  };
  ...
  public shared ({ caller }) func icrc2_approve({ from_subaccount : ?Subaccount; spender : Principal; amount : Int; expires_at : ?Nat64; memo : ?Memo; fee : ?Tokens; created_at_time : ?Timestamp }) : async Result<TxIndex, ApproveError> {
    ...
    let txid = recordTransaction({ operation = #Approve(approval); fee = effectiveFee; timestamp = now });
